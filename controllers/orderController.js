...
export async function createOrder (req, res) {
    ...
    Order.find().sort({ date: -1 }).limit(1).then(
        async (lastBills) => {
            if (lastBills.length == 0) {
                ...
                const newOrderId = "ORD" + newOrderNumberStr;
                orderData.orderId = newOrderId; // <-- FIXED: assign newOrderId
            }
            ...
            for (let i = 0; i < body.billItems.length; i++) {
                ...
                if (product == null) {
                    res.status(404).json({
                        message: "Product not found with id " + body.billItems[i].productId
                    });
                    return;
                }
